      ...
      try {
        ...
        if (!res.ok) {
          if (res.status === 404) {
            ...
            setCars([]);
            setFetchError(
              "ไม่พบข้อมูลกรมธรรม์ในระบบ กรุณาติดต่อเจ้าหน้าที่เพื่อตรวจสอบข้อมูล  ช่องทางติดต่อ: โทร 02-123-4567 หรืออีเมล support@example.com"
            ); return;
          }
          throw new Error(`โหลดข้อมูลรถไม่สำเร็จ (HTTP ${res.status})`);
        }
